...
export async function getOutletTimingsForRestaurant(restaurantId) {
    ...
    if (!doc) {
        const rest = await FoodRestaurant.findById(restaurantId).select('openingTime closingTime openDays').lean();
        if (rest) {
            ...
            const timings = DAY_NAMES.map((day) => {
                ...
                return {
                    day,
                    isOpen,
                    openingTime: isOpen ? normalizeTime(regOpening, '09:00') : '',
                    closingTime: isOpen ? normalizeTime(regClosing, '22:00') : ''
                };
            });
