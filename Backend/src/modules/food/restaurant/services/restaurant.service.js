...
const toRestaurantProfile = (doc) => {
    ...
    return {
        ...
        featuredPrice: Number.isFinite(Number(doc.featuredPrice)) ? Number(doc.featuredPrice) : null,
        costForTwo: Number.isFinite(Number(doc.featuredPrice))
            ? Number(doc.featuredPrice)
            : (Number.isFinite(Number(doc.costForTwo)) ? Number(doc.costForTwo) : null),
        ...
        estimatedDeliveryTimeMinutes:
            Number.isFinite(Number(doc.estimatedDeliveryTimeMinutes))
                ...
                : null,
        diningSettings: {
            isEnabled: Boolean(doc.diningSettings?.isEnabled),
            maxGuests: Math.max(1, parseInt(doc.diningSettings?.maxGuests, 10) || 6),
            diningType: Array.isArray(doc.diningSettings?.diningType)
                ? doc.diningSettings.diningType
                : (doc.diningSettings?.diningType 
                    ? String(doc.diningSettings.diningType).split(',').map(s => s.trim()).filter(Boolean)
                    : []),
            mealPeriods: Array.isArray(doc.diningSettings?.mealPeriods) && doc.diningSettings.mealPeriods.length > 0
                ? doc.diningSettings.mealPeriods
...
export const registerRestaurant = async (payload, files) => {
    ...
    const hasFileContent = (file) =>
        Boolean(file?.buffer) &&
        Number(file?.size || file?.buffer?.length || 0) > 0 &&
        Number(file?.buffer?.length || 0) > 0;
    ...
    try {
        ...
        const restaurant = await FoodRestaurant.create({
            ...
            externalReferralCode: String(externalReferralCode || referralCode || '').trim(),
            menuImages,
            menuPdf,
...
export const updateCurrentRestaurantDiningSettings = async (restaurantId, body = {}) => {
    ...
    const diningType =
        ...
        'family-dining';
    const mealPeriodsRaw = body.mealPeriods ?? currentDiningSettings.mealPeriods ?? ['breakfast', 'lunch', 'dinner'];
    const mealPeriods = Array.isArray(mealPeriodsRaw)
        ? [...new Set(mealPeriodsRaw.map((item) => String(item || '').trim().toLowerCase()).filter(Boolean))]
        : String(mealPeriodsRaw || '')
            .split(',')
            .map((item) => item.trim().toLowerCase())
            .filter(Boolean);
    const safeMealPeriods = mealPeriods.length > 0 ? mealPeriods : ['breakfast', 'lunch', 'dinner'];
...
export const uploadRestaurantCoverImages = async (restaurantId, files = []) => {
    ...
    const maxAllowed = 10 - existingCoverImages.length;
    if (maxAllowed <= 0) {
        throw new ValidationError('You can only upload up to 10 restaurant photos');
    }
    if (validFiles.length > maxAllowed) {
        throw new ValidationError(`You can only upload ${maxAllowed} more restaurant photos`);
    }

    const uploadedUrls = await Promise.all(
        validFiles.slice(0, 10).map((file) => uploadImageBuffer(file.buffer, 'food/restaurants/cover'))
    );
    ...
    const update = {
        coverImages: nextCoverImages.slice(0, 10)
    };
...
export const uploadRestaurantMenuImages = async (restaurantId, files = []) => {
    ...
    const maxAllowed = 10 - existingMenuImages.length;
    if (maxAllowed <= 0) {
        throw new ValidationError('You can only upload up to 10 menu photos');
    }
    if (validFiles.length > maxAllowed) {
        throw new ValidationError(`You can only upload ${maxAllowed} more menu photos`);
    }

    const uploadedUrls = await Promise.all(
        validFiles.slice(0, 10).map((file) => uploadImageBuffer(file.buffer, 'food/restaurants/menu'))
    );
    ...
    await FoodRestaurant.findByIdAndUpdate(
        ...
        {
            $set: {
                menuImages: nextMenuImages.slice(0, 10)
            },
            $unset: {
                rejectedAt: 1,
                rejectionReason: 1
...
export const getApprovedRestaurantByIdOrSlug = async (idOrSlug) => {
    ...
    const buildDiningSettings = (settings) => {
        return {
            isEnabled: settings?.isEnabled === true,
            maxGuests: Math.max(1, parseInt(settings?.maxGuests, 10) || 6),
            diningType: Array.isArray(settings?.diningType)
                ? settings.diningType
                : (settings?.diningType 
                    ? String(settings.diningType).split(',').map(s => s.trim()).filter(Boolean)
                    : []),
            mealPeriods: Array.isArray(settings?.mealPeriods) && settings.mealPeriods.length > 0
                ? settings.mealPeriods
                : ['breakfast', 'lunch', 'dinner']
        };
    };
    ...
    if (/^[0-9a-fA-F]{24}$/.test(value)) {
        ...
        if (!doc) return null;
        const hasFood = await FoodItem.exists({ restaurantId: doc._id, approvalStatus: 'approved' });
        if (!hasFood) return null;
        return {
            ...doc,
            rating: normalizeRatingValue(doc.rating),
            totalRatings: normalizeTotalRatingsValue(doc.totalRatings),
            diningSettings: buildDiningSettings(doc.diningSettings)
        };
    }
...
export const listPublicOffers = async () => {
    ...
    const allOffers = filteredList.map((o) => {
