...
export default function GroceryCategories() {
  ...
  // Using explicit back navigation to Grocery home
  const goBack = () => navigate("/food/user/under-250");
  ...
  const fetchCategories = useCallback(async () => {
    try {
      setLoading(true);
      const response = await groceryPublicAPI.getCategories();
      const list =
  ...
  const socketListeners = useMemo(() => ({
    'grocery:category:update': () => {
      console.log('[GroceryCategories] Category updated via socket, refetching...');
      fetchCategories();
    },
  }), [fetchCategories]);
    ...
    <div className="min-h-screen bg-white dark:bg-[#0a0a0a] pb-10">
      ...
      <div className="sticky top-0 z-50 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-md border-b border-neutral-100 dark:border-neutral-800 px-4 py-4 flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] text-neutral-500 dark:text-neutral-400 font-bold uppercase tracking-widest leading-none mt-1">Shop by Category</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-x-4 gap-y-10">
            {filteredCategories.map((category, index) => {
              return (
                ...
                >
                  <Link
                    to={`/food/user/under-250?category=${category.id}`}
