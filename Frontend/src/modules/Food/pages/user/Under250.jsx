...
import { Search, ChevronLeft, ChevronRight, Minus, Plus, ShoppingCart, X, Heart } from "lucide-react";
import AnimatedPage from "@food/components/user/AnimatedPage";
import { Card } from "@food/components/ui/card";
import { Button } from "@food/components/ui/button";
import { Input } from "@food/components/ui/input";
import PageNavbar from "@food/components/user/PageNavbar";
import { groceryPublicAPI, publicGetOnce } from "@food/api";
...
export default function GroceryPage() {
  ...
  const fetchBanners = async () => {
    try {
      ...
      const res = await publicGetOnce("/food/hero-banners/under-250/public");
      ...
      console.error("Failed to fetch under-250 banners", err);
      setDynamicBanners([]);
    } finally {
  ...
  const socketListeners = useMemo(() => ({
    ...
    'banner:update': (data) => {
      if (data?.section === 'grocery') {
        console.log('[Grocery] Banner updated via socket, refetching...');
        fetchBanners();
      }
    },
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-[#121212] pb-24 font-outfit">
        ...
        <div className="md:hidden sticky top-0 z-40 bg-[var(--module-theme-color,#F84E04)] shadow-md">
          <div className="flex items-center gap-3 p-4">
            ...
            <div>
              ...
              <p className="text-xs text-white/80 font-medium">Delivery in 10-15 mins</p>
            </div>
            <div className="ml-auto flex items-center gap-2.5">
              <HeaderNotificationBell className="w-5 h-5 text-white/95" triggerClass="h-9 w-9 rounded-lg" />
              <div ref={cartIconRef} className="bg-white/10 text-white p-2 rounded-lg flex items-center gap-2 relative cursor-pointer hover:bg-white/20 transition-colors" onClick={() => navigate("/food/user/cart?module=grocery")}>
        ...
        <div className="max-w-7xl mx-auto px-0 md:px-6 lg:px-8 py-0 md:py-6">
          ...
          {dynamicBanners.length > 0 && (
            ...
            <div className="w-full h-32 md:h-auto md:aspect-[21/9] lg:aspect-[2.5/1] rounded-2xl overflow-hidden relative shadow-[0_8px_20px_-4px_rgba(0,0,0,0.1)] bg-gray-100">
              ...
              {dynamicBanners.length > 1 && (
                <>
                  <button 
                    onClick={(e) => { e.stopPropagation(); setCurrentBanner((prev) => (prev - 1 + dynamicBanners.length) % dynamicBanners.length); }}
                  ...
                  <button 
                    onClick={(e) => { e.stopPropagation(); setCurrentBanner((prev) => (prev + 1) % dynamicBanners.length); }}
              ...
              <>
                ...
                {categories.length > 8 && (
                  <div 
                    ...
                    onClick={() => navigate("/food/user/under-250/categories")}
                  >
                    ...
                    <div className="relative pt-[100%] bg-gray-50 dark:bg-gray-800/50 p-2">
                      ...
                      <button
                        onClick={(e) => {
                          ...
                          const isFav = isDishFavorite(pId, null, "grocery");
                          if (isFav) {
                            removeDishFavorite(pId, null, "grocery");
                            toast.success("Removed from wishlist");
                          } else {
                            addDishFavorite({ ...product, type: "grocery" });
                      ...
                      >
                        <Heart
                          className={`w-3.5 h-3.5 ${
                            isDishFavorite(product.id || product._id, null, "grocery")
