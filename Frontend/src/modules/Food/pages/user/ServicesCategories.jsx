...
export default function ServicesCategories() {
  ...
  // Using explicit back navigation to Services home
  const goBack = () => navigate("/food/user/services");
  ...
  const socketListeners = useMemo(() => ({
    'services:category:update': () => {
      console.log('[ServicesCategories] Category updated via socket, refetching...');
      fetchCategories();
    },
  }), [fetchCategories]);
          ...
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-x-4 gap-y-10">
            {filteredCategories.map((category, index) => {
              return (
                ...
                >
                  <Link
                    to={`/food/user/services/${category.slug}?categoryId=${category.id}`}
