...
export default function AccessoriesProductsList() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const res = await accessoriesAdminAPI.bulkDeleteProducts(selectedIds)
      if (res.data?.success) {
        ...
        <DialogContent className="sm:max-w-[400px] p-6">
          ...
          <div className="py-2 pr-6">
            <p className="text-sm text-slate-600">
              Are you sure you want to delete {selectedIds.length} selected accessory(ies)? This action cannot be undone.
            </p>
          </div>
