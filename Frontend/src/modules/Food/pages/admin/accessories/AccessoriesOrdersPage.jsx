...
import { adminAPI, accessoriesAdminAPI } from "@food/api"
...
export default function accessoriesOrdersPage({ statusKey = "all" }) {
  ...
  const fetchOrders = useCallback(async (options = {}) => {
    ...
    try {
      ...
      const response = await accessoriesAdminAPI.getOrders(params)
      ...
      if (response.data?.success) {
        ...
        // Suppressing toast here to avoid double toasts
        setOrders([])
      }
    } catch (error) {
      debugError("Error fetching orders:", error)
      if (!silent) {
        // If the endpoint is not yet implemented, suppress the error
  ...
  const handleAcceptOrder = async (order) => {
    ...
    try {
      ...
      const response = await accessoriesAdminAPI.updateOrderStatus(orderIdToUse, { status: 'confirmed' })
      if (response.data?.success) {
  ...
  const handleUpdateStatus = async (order, status) => {
    ...
    try {
      setProcessingActionOrderId(order.id || order.orderId)
      const response = await accessoriesAdminAPI.updateOrderStatus(orderIdToUse, { status })
  ...
  const handleRejectOrder = async (order) => {
    ...
    try {
      setProcessingActionOrderId(order.id || order.orderId)
      const response = await accessoriesAdminAPI.updateOrderStatus(orderIdToUse, { status: 'cancelled_by_admin', reason })
      if (response.data?.success) {
  ...
  const handleDeassignAndResend = async (order) => {
    ...
    try {
      setProcessingActionOrderId(order.id || order.orderId)
      toast.info("Deassign and resend not implemented for accessories yet")
      // const response = await accessoriesAdminAPI.deassignAndResendOrder(orderIdToUse)
      // toast.success(
  ...
  const handleResendNotification = async (order) => {
    ...
    try {
      setProcessingActionOrderId(order.id || order.orderId)
      toast.info("Resend notification not implemented for accessories yet")
      // const response = await accessoriesAdminAPI.resendDeliveryNotification(orderIdToUse)
      // toast.success(
      //   response?.data?.message ||
  ...
  const processRefund = async (order, refundAmount = null) => {
    ...
    try {
      ...
      toast.info("Refunds not implemented for accessories yet")
