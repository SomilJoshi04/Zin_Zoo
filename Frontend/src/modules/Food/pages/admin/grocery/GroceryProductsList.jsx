...
export default function GroceryProductsList() {
  ...
  const fetchCategoriesForFilter = useCallback(async () => {
    try {
      const res = await groceryAdminAPI.getCategories({ limit: 1000 })
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const res = await groceryAdminAPI.bulkDeleteProducts(selectedIds)
      if (res.data?.success) {
  ...
  const fetchAllFoods = useCallback(async () => {
    try {
      ...
      const foodsRes = await groceryAdminAPI.getProducts(params)
  ...
  const socketListeners = useMemo(() => ({
    'grocery:product:update': () => {
      console.log('[GroceryList] Grocery products updated via socket, refetching...');
      fetchAllFoods();
    }
  }), [fetchAllFoods]);
  ...
  const handleFoodFormSubmit = async () => {
    ...
    try {
      ...
      if (foodFormMode === "edit") {
        res = await groceryAdminAPI.updateProduct(editingFood.id, payload)
      } else {
        ...
        res = await groceryAdminAPI.createProduct(payload)
      }
      toast.success(foodFormMode === "edit" ? "Grocery updated successfully" : "Grocery added successfully")
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setDeleting(true)
      const response = await groceryAdminAPI.deleteProduct(food?._id || food?.id)
    ...
    <div className="p-4 lg:p-6 bg-slate-50 min-h-screen">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Grocery Products</h1>
        </div>
      ...
      <Dialog open={showDetailModal} onOpenChange={setShowDetailModal}>
        <DialogContent className="max-w-xl p-0 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
          <DialogHeader className="px-6 py-4 border-b border-slate-200 bg-slate-50 dark:bg-slate-900/50 dark:border-slate-800">
            <DialogTitle className="text-lg font-semibold text-slate-900 dark:text-white">Grocery Details</DialogTitle>
          </DialogHeader>
          {selectedFood && (
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm bg-slate-50 border border-slate-200 rounded-lg p-4 dark:bg-slate-800 dark:border-slate-700">
                ...
                <p><span className="font-semibold text-slate-700 dark:text-slate-300">Grocery Type:</span> <span className="text-slate-900 dark:text-white">{selectedFood.foodType || "-"}</span></p>
              </div>
        ...
        <DialogContent className="max-w-2xl p-0 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
          <DialogHeader className="px-6 py-4 border-b border-slate-200 bg-slate-50 dark:bg-slate-900/50 dark:border-slate-800">
            <DialogTitle className="text-lg font-semibold text-slate-900 dark:text-white">
              {foodFormMode === "edit" ? "Edit Grocery" : "Add Grocery"}
            </DialogTitle>
          </DialogHeader>
          <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Grocery Name</label>
                <input
                  type="text"
                  value={foodForm.name}
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3 dark:bg-slate-900/50 dark:border-slate-800">
              ...
              {(foodForm.variants || []).length ? (
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">No variants added. This grocery item will use the single base price.</p>
              )}
            </div>
            <div className="flex justify-end">
              ...
              >
                ...
                <span>{submittingFood ? "Saving..." : foodFormMode === "edit" ? "Update Grocery" : "Add Grocery"}</span>
              </button>
            </div>
          </div>
        </DialogContent>
